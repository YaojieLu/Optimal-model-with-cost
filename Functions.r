...
averBf <- function(wL, parms, mul){
  with(as.list(c(wL, parms, mul)), {
    ...
    muf <- function(mu, wL){
      gswL <- gsBCf(wL, mu, wL)
      gs11 <- as.numeric(ode(y=c(gs=gswL), times=c(wL, 1), func=dgsdwf, parms=NULL, mu=mu, wL=wL)[2, 2])
      gs12 <- gsBCf(1, mu, wL)
      res <- gs11-gs12
      return(res^2)
    }
    ...
    gswf <- function(w, mu, wL){
      gswL <- gsBCf(wL, mu, wL)
      res <- as.numeric(ode(y=c(gs=gswL), times=c(wL, w), func=dgsdwf, parms=NULL, mu=mu, wL=wL)[2, 2])
      return(res)
    }
